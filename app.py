# app.py (UI/자동로드 원복, GDrive 로드 오류 수정, 상태 업데이트 오류(StreamlitAPIException) 콜백으로 수정)
...
STATE_KEYS_TO_SAVE = [
    ...
    # "prev_final_selected_vehicle", # 콜백 사용 시 불필요
...
def sync_move_type(widget_key):
    ...
    if widget_key in st.session_state:
        ...
        if st.session_state.base_move_type != new_value:
            ...
            # 이사 유형 변경 시 추천 차량 재계산 및 상태 업데이트 (선택적)
            ...
            #     st.session_state.recommended_vehicle_auto, _ = calculations.recommend_vehicle(st.session_state.total_volume, st.session_state.total_weight)
            #     # 이사 유형 변경 시 차량 선택 콜백도 호출하여 최종 차량 및 바구니 업데이트
            #     update_vehicle_and_baskets()
            # except Exception as e:
            #     print(f"Error recalculating on move type change: {e}")
            # st.rerun() # 필요 시 UI 즉시 업데이트

def update_vehicle_and_baskets():
    """(콜백 함수) 차량 선택 위젯 변경 시 최종 차량 결정 및 바구니 수량 업데이트"""
    # 1. 현재 위젯 상태 읽기
    vehicle_radio = st.session_state.get('vehicle_select_radio', "자동 추천 차량 사용")
    manual_vehicle = st.session_state.get('manual_vehicle_select_value') # Selectbox의 현재 값
    recommended_vehicle = st.session_state.get('recommended_vehicle_auto') # Tab 2에서 계산된 값
    ...
    if vehicle_radio == "자동 추천 차량 사용":
        ...
        # Selectbox에서 선택된 값이 유효한 트럭 목록에 있는지 확인
        if manual_vehicle in available_trucks:
            new_final_vehicle = manual_vehicle
        else:
            # 수동 선택 값이 유효하지 않을 경우 처리 (예: None 유지 또는 기본값 설정)
            # print(f"Warning: Invalid manual vehicle selection '{manual_vehicle}'")
            pass # 또는 기본값 설정 로직 추가

    # 3. 최종 선택 차량 상태 업데이트 (변경된 경우에만)
    if st.session_state.final_selected_vehicle != new_final_vehicle:
        st.session_state.final_selected_vehicle = new_final_vehicle

        # 4. 변경된 최종 차량에 따라 기본 바구니 수량 업데이트
        basket_section_name = "포장 자재 📦"
        current_move_type_auto = st.session_state.base_move_type
        basket_keys = [f"qty_{current_move_type_auto}_{basket_section_name}_{item}"
                       for item in data.default_basket_quantities.get(new_final_vehicle, {}).keys()]
        ...
        if new_final_vehicle and new_final_vehicle in data.default_basket_quantities:
            ...
            # 차량이 선택되지 않았거나 기본값 정보가 없는 경우, 기존 바구니 값 유지 또는 0으로 초기화 선택 가능
            # 예시: 기존 값 0으로 초기화
            # for item_key in basket_keys:
...
def initialize_session_state():
    ...
    defaults = {
        ...
        "is_storage_move": False, "storage_type": data.DEFAULT_STORAGE_TYPE if hasattr(data, 'DEFAULT_STORAGE_TYPE') else "",
        "apply_long_distance": False, "customer_name": "", "customer_phone": "",
        "from_location": "", "to_location": "", "moving_date": default_date,
        "from_floor": "", "from_method": data.METHOD_OPTIONS[0] if hasattr(data, 'METHOD_OPTIONS') else "",
        "to_floor": "", "to_method": data.METHOD_OPTIONS[0] if hasattr(data, 'METHOD_OPTIONS') else "",
        "special_notes": "", "storage_duration": 1,
        "long_distance_selector": data.long_distance_options[0] if hasattr(data, 'long_distance_options') else "",
        ...
        # "prev_final_selected_vehicle": None, # 콜백 사용 시 불필요
    ...
    for k in int_keys + float_keys:
        try:
            val = st.session_state.get(k, defaults.get(k)); target_type = int if k in int_keys else float
            ...
            st.session_state[k] = converted_val if k in allow_negative_keys else (max(0.0, converted_val) if k in float_keys else max(0, converted_val))
...
def prepare_state_for_save():
    ...
    # prev_final_selected_vehicle 키 제외하고 저장
...
def load_state_from_data(loaded_data):
    ...
    defaults_for_recovery = {
        "base_move_type": MOVE_TYPE_OPTIONS[0], "is_storage_move": False, "storage_type": data.DEFAULT_STORAGE_TYPE if hasattr(data, 'DEFAULT_STORAGE_TYPE') else "",
        "apply_long_distance": False, "customer_name": "", "customer_phone": "", "from_location": "",
        "to_location": "", "moving_date": date.today(), "from_floor": "", "from_method": data.METHOD_OPTIONS[0] if hasattr(data, 'METHOD_OPTIONS') else "",
        "to_floor": "", "to_method": data.METHOD_OPTIONS[0] if hasattr(data, 'METHOD_OPTIONS') else "",
        "special_notes": "", "storage_duration": 1,
        "long_distance_selector": data.long_distance_options[0] if hasattr(data, 'long_distance_options') else "",
        "vehicle_select_radio": "자동 추천 차량 사용", "manual_vehicle_select_value": None,
    ...
    # 로드 후 차량/바구니 상태 업데이트 콜백 명시적 호출
    update_vehicle_and_baskets() # 로드된 상태 기준으로 콜백 실행하여 초기 상태 반영
...
with tab1:
    ...
    with st.container(border=True):
        ...
        with col_load: # 불러오기
            ...
            if st.session_state.gdrive_search_results:
                ...
                # 파일 선택 selectbox
                # Selectbox 자체의 값 변경은 on_change 콜백에서 처리하지 않고, 아래 버튼 클릭 시 최종 ID를 사용
                selected_filename_widget = st.selectbox(
                    "불러올 파일 선택:", options=file_options_display,
                    key="gdrive_selectbox_widget", # 위젯 키 분리
                    index=current_index
                )
                # Selectbox에서 선택된 파일 이름과 ID를 임시로 session_state에 저장 (버튼 클릭 시 사용)
            ...
            if st.button("📂 선택 견적 불러오기", disabled=load_button_disabled, key="load_gdrive_btn"):
                file_id = st.session_state.gdrive_selected_file_id # Selectbox에서 최종 선택된 ID 사용
                if file_id:
                    ...
                    if loaded_data:
                        ...
                        if load_success:
                            ...
                            # 로드 후에는 콜백이 이미 호출되었으므로 rerun 불필요할 수 있음
                            st.rerun()
                    # 오류 메시지는 함수 내부에서 처리
    ...
    with col2:
        ...
        st.selectbox("🛠️ 도착지 작업 방법", method_options, key="to_method", help="사다리차, 승강기, 계단, 스카이 중 선택")
    ...
    if st.session_state.get('is_storage_move'):
        ...
        storage_options = getattr(data, 'STORAGE_TYPE_OPTIONS', ["선택"])
        if storage_options and storage_options[0] != "선택":
...
# =============================================================================
# === Vehicle Selection Logic (콜백 방식으로 변경됨) ===
# 이 섹션은 콜백 함수 update_vehicle_and_baskets 로 대체되었으므로 주석 처리 또는 삭제
# =============================================================================
# prev_vehicle = st.session_state.get('prev_final_selected_vehicle')
# ... (기존 vehicle_changed_flag 및 직접 할당 로직) ...
        ...
        with col_v2_widget:
            ...
            if not use_auto_widget or (use_auto_widget and not valid_auto_widget):
                if not available_trucks_widget: st.error("❌ 선택 가능 차량 정보 없음.")
                    ...
                    # --- StreamlitAPIException 수정: on_change 콜백 연결 ---
                    # Selectbox의 현재 값을 state와 동기화하고, 변경 시 콜백 호출
                    st.selectbox("차량 직접 선택:", available_trucks_widget, index=idx,
                                 key="manual_vehicle_select_value", # Selectbox 자체의 값을 저장할 키
                                 on_change=update_vehicle_and_baskets) # 변경 시 콜백 호출
                    # -----------------------------------------------------

                    manual_selected = st.session_state.get('manual_vehicle_select_value') # 현재 Selectbox 값
                    # 수동으로 선택된 차량 정보를 표시 (final_selected_vehicle이 아닌 manual_selected 기준)
                    if manual_selected:
