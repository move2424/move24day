# app.py (UI/자동로드 원복, GDrive 로드 AttributeError 수정 - load_file 사용)
...
# 콜백 방식 상태 업데이트 로직 (StreamlitAPIException 해결 위함 - 주석 처리됨)
# def update_vehicle_and_baskets(): ... # (이전 코드 내용)
...
def prepare_state_for_save():
    ...
    actual_keys_to_save = list(set(STATE_KEYS_TO_SAVE + ['prev_final_selected_vehicle']) - widget_keys_to_exclude) # prev 저장 포함
    for key in actual_keys_to_save:
        if key in st.session_state:
            ...
            if isinstance(value, date): state_to_save[key] = value.isoformat()
                 ...
                 except Exception as e: print(f"Warning: Skipping key '{key}'. Error: {e}")
...
def load_state_from_data(loaded_data):
    ...
    defaults_for_recovery = {
        ...
        "long_distance_selector": data.long_distance_options[0] if hasattr(data, 'long_distance_options') else "",
        "vehicle_select_radio": "자동 추천 차량 사용", "manual_vehicle_select_value": None,
        "final_selected_vehicle": None, "prev_final_selected_vehicle": None, # prev 로드 포함
    ...
    all_expected_keys = list(set(STATE_KEYS_TO_SAVE + ['prev_final_selected_vehicle'])) # prev 포함
...
with tab1:
            ...
            if st.button("📂 선택 견적 불러오기", disabled=load_button_disabled, key="load_gdrive_btn"):
                ...
                if file_id:
                    with st.spinner(f"🔄 견적 파일 로딩 중..."):
                        ...
                        # loaded_images 변수는 더 이상 사용되지 않음
                        # -----------------------------
                    if loaded_data:
        ...
        with col_save: # 저장
            ...
            if st.button("💾 Google Drive에 저장", key="save_gdrive_btn"):
                ...
                if phone_part == "번호없음" or len(phone_part) < 4 or not str(st.session_state.get('customer_phone', '')).strip():
                    ...
                    else: st.error(f"❌ '{save_filename}' 파일 저장 중 오류 발생.") # gdrive_utils 내부 오류 메시지 외 추가 표시
    ...
    with col1:
        ...
        if st.session_state.get('apply_long_distance'):
            # data.long_distance_options 사용 확인
    ...
    if st.session_state.get('is_storage_move'):
        ...
        # data.STORAGE_TYPE_OPTIONS 사용 확인
...
# =============================================================================
# === Vehicle Selection Change Detection & Auto-Basket Logic ===
# (StreamlitAPIException 관련 콜백 방식은 주석 처리 - 기존 방식 유지)
# =============================================================================
prev_vehicle = st.session_state.get('prev_final_selected_vehicle')
# current_final_vehicle = st.session_state.get('final_selected_vehicle') # 이 변수는 아래 로직에서 결정됨
...
if selected_vehicle_logic != prev_vehicle:
    # prev_vehicle이 초기값(None)이거나, 이전 루프에서 업데이트된 값일 수 있음
    # 직접 할당 방식에서는 prev_final_selected_vehicle 상태를 계속 추적해야 함
    if st.session_state.get('prev_final_selected_vehicle') == prev_vehicle:
        st.session_state.final_selected_vehicle = selected_vehicle_logic
        st.session_state.prev_final_selected_vehicle = selected_vehicle_logic # 다음번 비교를 위해 업데이트
        vehicle_changed_flag = True
        # 바구니 업데이트 로직 (기존 방식 유지 - StreamlitAPIException 발생 가능성 있음)
        if selected_vehicle_logic and selected_vehicle_logic in data.default_basket_quantities: # None 체크 추가
            ...
            for item, qty in defaults.items():
                ...
                if key in st.session_state:
                    st.session_state[key] = qty # 여기가 잠재적 문제 지점
    else: # 연속 변경 등으로 상태가 꼬였을 경우 현재 로직값으로 강제 동기화
        st.session_state.final_selected_vehicle = selected_vehicle_logic
        st.session_state.prev_final_selected_vehicle = selected_vehicle_logic # 다음번 비교를 위해 업데이트

elif st.session_state.final_selected_vehicle != selected_vehicle_logic :
     # 위젯 상태는 바뀌었지만 prev_vehicle과 같아서 위 조건에 걸리지 않은 경우
     st.session_state.final_selected_vehicle = selected_vehicle_logic
     # prev_final_selected_vehicle는 다음 루프에서 업데이트 되도록 둠


# prev_final_selected_vehicle 상태가 현재 final_selected_vehicle과 다르면 업데이트
if st.session_state.prev_final_selected_vehicle != st.session_state.final_selected_vehicle:
...
# --- 탭 3: 견적 및 비용 ---
# (이전 코드와 동일하게 유지 - 변경 없음, 단 콜백 연결은 제거됨)
with tab3:
    st.header("💰 계산 및 옵션 ")
    # 이사 유형 확인/변경
    ...
    with st.container(border=True):
        ...
        with col_v1_widget:
            st.radio("차량 선택 방식:", ["자동 추천 차량 사용", "수동으로 차량 선택"],
                     key="vehicle_select_radio", # on_change 제거
            ...
            if not use_auto_widget or (use_auto_widget and not valid_auto_widget):
                if not available_trucks_widget: st.error("❌ 선택 가능 차량 정보 없음.")
                    ...
                    st.selectbox("차량 직접 선택:", available_trucks_widget, index=idx,
                                 key="manual_vehicle_select_value") # on_change 제거
    ...
    # 차량 변경 시 자동 바구니 업데이트 후 rerun (주석 처리됨)
