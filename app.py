# app.py (UI/자동로드 원복, 이사 유형 선택만 동기화, GDrive 로드 오류 및 상태 업데이트 오류 수정)
...
# =============================================================================
# === Vehicle Selection and Auto-Basket Logic (StreamlitAPIException Fix) ===
# =============================================================================
# 차량 선택 관련 상태 가져오기
...
if selected_vehicle_logic != prev_vehicle:
    ...
    if prev_vehicle == prev_prev_vehicle_state:
        ...
        # --- 수정 시작 (StreamlitAPIException Fix) ---
        # 차량 변경 시 기본 바구니 수량 자동 업데이트
        if selected_vehicle_logic in data.default_basket_quantities:
            ...
            for item, qty in defaults.items():
                ...
                # 방어 코드: 키가 유효한 문자열이고 session_state에 존재하는지 확인 후 할당
                if isinstance(key, str) and key.strip():
                     if key in st.session_state:
                         st.session_state[key] = qty
                     # else:
                         # 키가 존재하지 않는 경우 (초기화 로직 문제 등) 경고 출력 가능
                         # print(f"Warning: Key '{key}' not found in session_state during basket update.")
                # else:
                    # print(f"Warning: Invalid key '{key}' generated during basket update.")
        # --- 수정 끝 ---
